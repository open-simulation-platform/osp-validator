...
public class VE_AngularDisplacement_2 extends ValidationError<AngularDisplacement> {
  ...
  protected List<AngularDisplacement> getInvalidObjects() {
    OWLClass validationErrorClass = context.owl.dataFactory.getOWLClass(VE_AngularDisplacement_2, context.owl.prefixManager);
  ...
  protected String getErrorMessage(AngularDisplacement angularDisplacement) {
    return "AngularDisplacement variable group '" + angularDisplacement.getName().getId().toString() + "' contains variables with mixed data types";
  }
}
