...
public class VE_Voltage_3 extends ValidationError<Voltage> {
  ...
  protected List<Voltage> getInvalidObjects() {
    ...
    OWLClass validationErrorClass = config.dataFactory.getOWLClass(VE_Voltage_3, config.prefixManager);
  ...
  protected String getErrorMessage(Voltage voltage) {
    return "Voltage variable group '" + voltage.getName().get() + "' contains variables with incompatible units";
  }
}
